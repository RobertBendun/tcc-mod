...
void download(char const *url, char *download_path)
{
	static CURL *handle = NULL;
	char buf[1024];
	char const *cache, *dir, *name, *path;
	FILE *f;
	int sz;
	CURLcode res;

	path  = tcc_strdup(url + 8);
	cache = getenv("XDG_CACHE_HOME");
	name  = basename((char*)path);
	dir   = dirname((char*)path);

	sz = snprintf(buf, sizeof(buf), "%s/tcc/%s/", cache, dir);
	if (mkpath(buf, 0755) < 0)
		tcc_error("Could not make dir tree for soon downloaded file");
	...
	strncat(buf, name, sizeof(buf) - sz);
	f = fopen(buf, "wb");
